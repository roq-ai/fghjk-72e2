...
import { getResults } from 'apiSdk/results';
...
function DashboardPage() {
  ...
  const resultStat = useSWR<PaginatedInterface<ResultInterface>>(
    () => `/results`,
    () => getResults({ limit: 1, offset: 0 }),
  );

  const excelDataStat = useSWR<PaginatedInterface<ExcelDataInterface>>(
    () => `/excel-data`,
    () => getExcelData({ limit: 1, offset: 0 }),
  );

  const algorithmStat = useSWR<PaginatedInterface<AlgorithmInterface>>(
    () => `/algorithms`,
    () => getAlgorithms({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Results',
      result: resultStat?.data?.totalCount,
      isLoading: resultStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Excel Data',
      result: excelDataStat?.data?.totalCount,
      isLoading: excelDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Algorithms',
      result: algorithmStat?.data?.totalCount,
      isLoading: algorithmStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
