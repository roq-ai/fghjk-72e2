...
type ColumnType = ColumnDef<ResultInterface, unknown>;
...
export function ResultListPage(props: ResultListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getResults({
        relations: ['algorithm', 'excel_data'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['result_data.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ResultInterface>>(
    () => `/results?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ResultInterface) => {
    if (hasAccess('result', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/results/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'result_data', header: 'Result Data', accessorKey: 'result_data' },
    hasAccess('algorithm', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'algorithm',
          header: 'Algorithm',
          accessorKey: 'algorithm',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/algorithms/view/${record.algorithm?.id}`}>
              {record.algorithm?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('excel_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'excel_data',
          header: 'Excel Data',
          accessorKey: 'excel_data',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/excel-data/view/${record.excel_data?.id}`}>
              {record.excel_data?.data}
            </Link>
          ),
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/results/view/${record.id}`} passHref legacyBehavior>
                <Button
