...
import { getResultById } from 'apiSdk/results';
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('result', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/results/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Result Data" text={data?.result_data} />
                ...
                {hasAccess('algorithm', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Algorithm"
                    text={
                      <Link as={NextLink} href={`/algorithms/view/${data?.algorithm?.id}`}>
                        {data?.algorithm?.name}
                      </Link>
                    }
                  />
                ...
                {hasAccess('excel_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Excel Data"
                    text={
                      <Link as={NextLink} href={`/excel-data/view/${data?.excel_data?.id}`}>
