...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getExcelData() {
    ...
    const response = await prisma.excel_data
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'excel_data'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createExcelData() {
    ...
    if (body?.result?.length > 0) {
      const create_result = body.result;
      body.result = {
    ...
    const data = await prisma.excel_data.create({
      data: body,
    });
