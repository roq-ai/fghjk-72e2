...
import { getAlgorithmById } from 'apiSdk/algorithms';
import { AlgorithmInterface } from 'interfaces/algorithm';
import { ResultListPage } from 'pages/results';

function AlgorithmViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AlgorithmInterface>(
    () => (id ? `/algorithms/${id}` : null),
    () =>
      getAlgorithmById(id, {
        relations: ['user'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('algorithm', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/algorithms/edit/${id}`} passHref legacyBehavior>
                    <Button
