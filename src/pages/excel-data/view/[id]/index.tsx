...
import { getExcelDataById } from 'apiSdk/excel-data';
import { ExcelDataInterface } from 'interfaces/excel-data';
import { ResultListPage } from 'pages/results';

function ExcelDataViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ExcelDataInterface>(
    () => (id ? `/excel-data/${id}` : null),
    () =>
      getExcelDataById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('excel_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/excel-data/edit/${id}`} passHref legacyBehavior>
                    <Button
